...
class TimeTrialListViewHolder(binding: ListItemTimetrialBinding): GenericBaseHolder<TimeTrial, ListItemTimetrialBinding>(binding) {
    private val _binding = binding

    override fun bind(vm: TimeTrial){
        _binding.apply{
            viewModel = vm
            executePendingBindings()
        }
...
class TimeTrialViewHolderFactory: GenericViewHolderFactory<TimeTrial>() {
    override fun createTitle(layoutInflator: LayoutInflater, parent: ViewGroup?): View {
        return createView(layoutInflator, parent, TimeTrial.createBlank().apply {
            ttName = "Time Trial Name"
            course = Course.createBlank().apply { courseName = "Course Name" }
        })
    }

    override fun createView(layoutInflator: LayoutInflater, parent: ViewGroup?, data: TimeTrial): View {
    ...
    override fun createViewHolder(layoutInflator: LayoutInflater, parent: ViewGroup?): GenericBaseHolder<TimeTrial, ListItemTimetrialBinding> {
