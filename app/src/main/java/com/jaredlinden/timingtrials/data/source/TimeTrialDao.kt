...
abstract class TimeTrialDao(db: RoomDatabase) {
    ...
    @Transaction @Query("SELECT * FROM timetrial_table WHERE courseId = :cId") abstract fun getTimeTrialsOnCourse(cId: Long): List<TimeTrialHeader>
    ...
    @Query("SELECT * FROM timetrial_rider_table WHERE timeTrialId = :timeTrialId ORDER BY `index`") abstract fun getTimeTrialRiders(timeTrialId: Long): List<TimeTrialRider>
