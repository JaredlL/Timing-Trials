...
class TestViewModel@Inject constructor(
    ...
    fun testSetup(){
            ...
            if(!inserting.get()){// && timeTrial.timeTrialHeader.ttName != "Test Setup Timetrial"){
                ...
                viewModelScope.launch(Dispatchers.IO) {
                    ...
                    timeTrialRepository.insert(copy)
                    inserting.set(false)
                    insertedTestTiming.postValue(Event(true))
                }
            }
    }
    ...
    fun testTiming(){
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val exist = timeTrialRepository.getTimeTrialByName("Test Timing TT")
            if(exist == null){

                val rList = riderRepository.allRidersLightSuspend().take(8)
                val courses = courseRepository.getAllCoursesSuspend()

                val _mTimeTrial =TimeTrial.createBlank().copy(
                        timeTrialHeader = TimeTrialHeader.createBlank()
                                .copy(ttName = "Test Timing TT",
                                        startTime = OffsetDateTime.ofInstant(Instant.now().truncatedTo(ChronoUnit.SECONDS).plusSeconds(3), ZoneId.systemDefault()),
                                        firstRiderStartOffset = 0,
                                        interval = 5,
    ...
    fun insert1000(){
        viewModelScope.launch(Dispatchers.IO) {
            for (i in 1..1000){
                val r = Rider(i.toString(), i.toString())
    ...
    fun insertFinishedTt3(){
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            val exist = timeTrialRepository.getTimeTrialByName("Test Result 3")
            if(exist == null){

                val rList = riderRepository.allRidersLightSuspend().take(5)
                val courses = courseRepository.getAllCoursesSuspend()
